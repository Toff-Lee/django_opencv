...
def simple_upload(request):
    ...
    if request.method == 'POST':
        form = SimpleUploadForm(request.POST, request.FILES) # field forms

        if form.is_valid():
            myfile = request.FILES['image']
            fs = FileSystemStorage()
            # myfile.name : "beatles.jpg" 파일명의 스트
            # myfile : beatles.jpg 파일 자체
            filename = fs.save(myfile.name, myfile) # "ses.jpg"
...
def detect_face(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            post = form.save(commit=False)  # filled form
             # 얼굴과 눈을 찾는 예측모델 적용 타이
            post.save()
